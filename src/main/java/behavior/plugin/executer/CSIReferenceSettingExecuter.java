 ...
 */
public class CSIReferenceSettingExecuter extends ReferenceSettingFrame{
	private String projectID;
	private String sep = System.getProperty("file.separator");
	private String programPath = (IJ.isWindows()? "" : System.getProperty("user.home"))+System.getProperty("file.separator")+"Image_CSI";
	private String referencePath;
	private String referenceFileName;
	//private Logger log = Logger.getLogger("behavior.plugin.executer.RMReferenceSettingExecuter");

	public CSIReferenceSettingExecuter(){
		super();
		setTitle("CSI Reference Setup");
		/*try{
    	...
    	public void preprocess(){
    		...
    		File[] list = path.listFiles(new FileFilter(){
    			public boolean accept(File pathname){
    				...
    				if(name.length() >= 5 && name.substring(name.length()-4).equals(".txt"))	// .txt なファイルのみ受け付ける
    					return true;
    				else
    					return false;
    			}
    ...
    private class SetupDialog extends ReferenceDialogPanel{
    	...
    	public void preprocess(){
    		...
    		table.getColumn("SubjectID").setCellEditor(new DefaultCellEditor(new ExtendedJTextField("", 10)));
    		table.getColumn("LeftCage").setCellEditor(new DefaultCellEditor(new ExtendedJTextField("", 10)));
    		table.getColumn("RightCage").setCellEditor(new DefaultCellEditor(new ExtendedJTextField("", 10)));
    	...
    	private DefaultTableModel createTableModel(){
    		String[] column = {"SubjectID", "LeftCage", "RightCage"};
    	...
    	public void save(){
			...
			for(int i = 0; i < tableModel.getRowCount(); i++){
				...
				row[1] = tableModel.getValueAt(i,1).toString().trim();
				row[2] = tableModel.getValueAt(i,2).toString().trim();
				//log.log(Level.INFO, row[0]+" "+row[1]);
				buf.append(row[0]).append("\t").append(row[1]).append("\t").append(row[2]);
    	...
    	public boolean canGoNext(){
    		...
    		for(int i = 0; i < tableModel.getRowCount(); i++){
    			if(!FilenameValidator.validate(tableModel.getValueAt(i,0).toString())){
    				BehaviorDialog.showErrorDialog(this, "Some SubjectID have a invalid name.\nMouse name must not be empty, and must not contain : \n\\ / : * ? \" < > |");
    				return false;
    			}
    			if(!FilenameValidator.validate(tableModel.getValueAt(i,1).toString())){
    				BehaviorDialog.showErrorDialog(this, "Some LeftCage have a invalid name.\nMouse name must not be empty, and must not contain : \n\\ / : * ? \" < > |");
    				return false;
    			}
    			if(!FilenameValidator.validate(tableModel.getValueAt(i,2).toString())){
    				BehaviorDialog.showErrorDialog(this, "Some RightCage have a invalid name.\nMouse name must not be empty, and must not contain : \n\\ / : * ? \" < > |");
    				return false;
    			}
    		}
